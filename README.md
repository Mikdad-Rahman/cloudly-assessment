...
The system ships with two UI options:

### HTML/JS UI v2.0 (Docker — Recommended)
A lightweight, Docker-native UI served by nginx. Instant load, no Python startup time.
Calls the FastAPI backend directly via REST.
Open http://localhost:3001

### Streamlit UI (Local)
A Python-based interactive UI. Works best running locally while Docker services are running.
During development, a startup timing issue was discovered with ChromaDB initialization
inside Docker on Windows/WSL2 — the HTML/JS UI was built as a Docker-native alternative.
...
- Streamlit UI has a startup timing issue with ChromaDB inside Docker on
  Windows/WSL2 — use the HTML/JS UI at http://localhost:3001 instead
