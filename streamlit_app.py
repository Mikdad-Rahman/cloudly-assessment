...
from app.db.database import _is_postgres_available, init_db, get_prior_sessions, get_kb_snapshot, get_weak_areas
...
if sections is None:
    st.error("⚠️ PostgreSQL is not running. Please start Docker first:")
...
# ========== PREP SESSION PAGE ==========
    ...
    if "result" not in st.session_state:
        ...
        if st.button("🚀 Start Prep Session", disabled=len(selected) == 0):
            ...
            if prior:
                ...
                if weak:
                    st.info("🎯 **Adaptive Mode** — Focusing on your weak areas:")
                    for w in weak[:5]:
    ...
    if (
        ...
        if result.get("is_adaptive"):
            ...
            st.info("📖 **First Session** — Fresh questions from selected sections")
        ...
        with st.form("answer_form"):
            ...
            for i, q in enumerate(questions, 1):
                st.markdown(f"**Q{i} [Section {q['section_id']}]:** {q['question_text']}")
        ...
        if submitted:
            if len(answers) < len(questions):
                st.warning(f"Please answer all questions. ({len(answers)}/{len(questions)} answered)")
            else:
                ...
                update_session_score(result["session_id"], scored["score"], scored["total"])
    ...
    if st.session_state.get("submitted") and "result" in st.session_state:
        ...
        if scored:
            ...
            col3.metric("Adaptive Mode", "ON ✅" if result.get("is_adaptive") else "OFF")
            ...
            for i, r in enumerate(scored["results"], 1):
                if r["is_correct"]:
                    st.success(f"**Q{i}:** {r['question_text']}\n\n✓ Correct — **{r['correct_answer']}**")
                else:
                    wrong_count += 1
                    st.error(
            ...
            if wrong_count > 0:
                ...
                st.markdown("These topics will be prioritized in your **next session**:")
                for r in scored["results"]:
                    if not r["is_correct"]:
...
# ========== HISTORY PAGE ==========
    ...
    section_input = st.number_input("Enter section ID to view history:", min_value=1, max_value=10, value=1)
    if st.button("Load History"):
        ...
        if not sessions:
            ...
            for s in sessions:
                ...
                with st.expander(f"Session {s['id']} — {s['created_at']} — Score: {score}/{total} ({pct}%)"):
                    st.json(s)
...
# ========== KB SNAPSHOT PAGE ==========
    ...
    if not snapshot:
        ...
        for s in snapshot:
            ...
            total = s['total_questions']
            pct = round((score/total)*100) if total > 0 else 0
            with st.expander(f"Session {s['id']} — Sections: {s['section_ids']} — Score: {score}/{total} ({pct}%)"):
                st.json(s)